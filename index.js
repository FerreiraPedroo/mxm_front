...
app.get("/rubricas", async (req, res) => {
 const caminhoArquivo = path.join(__dirname, 'public/dados','rubricas.json');
 const fileInfo = await carregarArquivo(rubricasPathcaminhoArquivo);
 return res.status(200).send(fileInfo);
});

app.get("/orcamento-valores", async (req, res) => {
 const caminhoArquivo = path.join(__dirname, 'public/dados','orcamento_valores.json');
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
app.post('/requisicao-novo', async (req, res) => {
 const caminhoArquivo = path.join(__dirname, 'public/dados','requisicoes.json');
 const fileInfo = await carregarArquivo(caminhoArquivo);
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
app.get("/requisicao-selecionada/:requisicaoID", async (req, res) => {
 try {
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// ### REQUISIÇÃO SELECIONADA: LISTA DE NOTAS FISCAIS DA REQUISIÇÃO
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
app.get("/requisicao-selecionada/:requisicaoID/notas-fiscais", async (req, res) => {
 try {
 const { requisicaoID } = req.params;
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// ### REQUISIÇÃO SELECIONADA: SALVAR NOTA FISCAL DA REQUISIÇÃO
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
app.post("/requisicao-selecionada/:requisicaoID/nota-fiscal", async (req, res) => {
 try {
 const { requisicaoID } = req.params;
 ...
 if(nfExistent) {
  throw {codStatus: 422, message:"A nota fiscal desse fornecedor já foi cadastrada.", data:""}
 }
 ...
 res.status(200).send({ codStatus: 200, message: "OK", data: nfsInfo }); 

 } catch(err) {
  if(err.codStatus) {
   res.status(err.codStatus).send(err)
  }
  res.status(500).send(err);
 }
});
